...
public class WeatherApi {
    ...
    public LongLat retrieveLongLat(String city, String stateCode) throws URISyntaxException, IOException, InterruptedException {
        ...
        HttpRequest httpRequest = HttpRequest.newBuilder(new URI("http://api.openweathermap.org/geo/1.0/direct?q=" + cityStateCountry + "&limit=1&appid=" + getApiKey())).GET().build();
    ...
    public JsonObject retrieveWeatherByLongLat(LongLat longLat) throws IOException, InterruptedException, URISyntaxException {
        HttpRequest httpRequest = HttpRequest.newBuilder(new URI("http://api.openweathermap.org/data/2.5/forecast?lat=" + longLat.getLatitude() + "&lon=" + longLat.getLongitude() + "&appid=" + getApiKey() + "&units=imperial")).GET().build();
        HttpResponse<String> response = client.send(httpRequest, HttpResponse.BodyHandlers.ofString());
